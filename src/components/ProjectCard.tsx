  ...
  return (
    <section id={project.id}>
      ...
      <div>
        ...
        {/* 一句话定位 */}
        <p className="mt-4 max-w-[600px] text-[17px] leading-relaxed text-text-secondary">
          {project.subtitle}
        </p>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-3">
          {project.keyMetrics.map((m) => (
            <div key={m.label}>
              ...
              <p className="mt-1 text-[17px] leading-relaxed text-text">
                {m.value}
              </p>
            </div>
          ))}
        ...
        {/* 辅助数据图 */}
        {project.keyDataImages.length > 0 && (
        ...
        <div className="mt-8 space-y-14">
          {project.details.map((section) => (
            <div key={section.heading}>
              <h3 className="text-[20px] leading-snug text-text">
                {section.heading}
              </h3>
